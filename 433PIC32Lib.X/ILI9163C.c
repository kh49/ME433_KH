...
void LCD_type(unsigned char x,unsigned char y,int *characters, unsigned short color) {
    char ii = 0;
    char length = sizeof(characters);
    if (((x+length*5-1)>128)|((y+8)>128)){
    }
    else {
    
    while(ii<length) {
        
        LCD_char(x,y,30,color);
        ii++;
        x = x+5;
    }
...
void LCD_char(unsigned char x, unsigned char y, int letter, unsigned short color) {
    char ii = 0;
    char jj = 0;
    char column = 0;
    while (ii<5){
        column = ASCII[letter][ii];
        while (jj<8){
            if ((column>>jj)&0b1){
